...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('mystats-manual')
        .setDescription('Shows your manual-claim stats for this server')
    ...
    async execute(interaction, { database }) {
        ...
        const cooldownKey = `${userId}-${guildId}-manual`;
        ...
        try {
            ...
            if (!userData || !userData.manualClaims) {
                return await handleInteraction(interaction, {
                    content: 'No manual claim data found for you in this server.',
                    ephemeral: true
                }, 'editReply');
            }
            ...
            // Use countManualClaims for tier counts
            const tierCounts = {
                CT: userData.countManualClaims[0] || 0,
                RT: userData.countManualClaims[1] || 0,
                SRT: userData.countManualClaims[2] || 0,
                SSRT: userData.countManualClaims[3] || 0,
            ...
            const embed = new EmbedBuilder()
                .setColor('#FFC0CB')
                .setAuthor({
                    name: `${interaction.user.username}'s Manual-Claim Stats`,
                    iconURL: interaction.user.displayAvatarURL(),
                    url: `https://mazoku.cc/user/${interaction.user.id}`
                })
                .setThumbnail(interaction.user.displayAvatarURL())
                .setFooter({ 
                    text: `Mazoku stats Manual-Summon | Player: ${interaction.user.username} | Guild: ${interaction.guild.name}` 
                });
            ...
            switch(statType) {
                case 'overview':
                    embed.addFields({ 
                        name: `Total Manual Claims:  ${totalClaims.toString()}`, 
                        value: `*Manual claims in last 30 minutes*: ${recentClaimsCount.toString()}`,
                    });
                    break;
                    ...
                    if (bestCard) {
                        try {
                            ...
                            if (enrichedCard && enrichedCard.card) {
                                ...
                                embed.addFields({
                                    name: 'Best Manual Drop (Last 30 Minutes)',
                                    value: `*${series}*\n` +
                                ...
                                embed.addFields({
                                    name: 'Best Manual Drop (Last 30 Minutes)',
                                    value: '*No manual drops in the last 30 minutes*'
                                });
                            }
                        } catch (error) {
                ...
                case 'tiers':
                    embed.addFields({
                        ...
                        value: Object.entries(tierCounts)
                            ...
                            .join('\n') || '*No manual claims data available*'
                    });
                    break;

                case 'prints':
                    embed.addFields({
                        name: 'Manual Print Distribution (Last 30 Minutes)',
                        value: Object.entries(printRangeCounts)
                            ...
                            .join('\n') || '*No manual print data available for the last 30 minutes*'
                    });
                    break;

                case 'tiertimes':
                    embed.addFields({
                        name: 'Average Time Between Manual Claims by Tier',
                        value: Object.entries(claimTimesByTier)
                            ...
                            .join('\n') || '*No manual claim data available*'
                    });
                    break;

                case 'printtimes':
                    embed.addFields({
                        name: 'Average Manual Print Claim Time',
                        value: Object.entries(claimTimesByPrintRange)
                            ...
                            .join('\n') || '*No manual print claim data available*'
                    });
                    break;
            }
            ...
            console.error('Error in mystats-manual command:', error);
            
            const errorMessage = error.message === "The Mazoku Servers are currently unavailable. Please try again later."
                ? error.message
                : 'An error occurred while fetching manual claim stats.';
